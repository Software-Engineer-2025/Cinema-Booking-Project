  ...
  return (
    <>
      ...
      {isOpen ? (
        <div
          className={
            "fixed bottom-5 right-5 w-[90dvw] h-[70dvh] rounded-md bg-neutral-700 shadow-lg" +
...
function PreBookForm({ bookableMovies }: { bookableMovies: Movie[] }) {
  ...
  const handleClick = () => {
    if (selectedMovie === null || selectedShowtime === null) {
      ...
      router.push(
        `/book?movieId=${selectedMovie?.movie_id}&showtime=${encodeURIComponent(selectedShowtime)}`,
      );
    }
  };
